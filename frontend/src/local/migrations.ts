...
const MIGRATIONS: Migration[] = [
  ...
  {
    version: 13,
    name: 'ensure_rest_timer_and_body_progress_recorded_date',
    apply: async (db) => {
      const routineColumns = await db.getAllAsync<{ name: string }>(
        `PRAGMA table_info(${LOCAL_TABLES.routines})`
      );
      const hasRestTime = routineColumns.some((column) => column.name === 'rest_time_seconds');
      ...
      const bodyProgressColumns = await db.getAllAsync<{ name: string }>(
        `PRAGMA table_info(${LOCAL_TABLES.bodyProgress})`
      );
      const hasRecordedDate = bodyProgressColumns.some((column) => column.name === 'recorded_date');
